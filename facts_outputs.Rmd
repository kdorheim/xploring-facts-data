...
```{r fig.cap = 'Figure 2. The fittedismip.ssp585 experiment, plotted with 2000 climate input samples from FaIR and Hector. The experiment emulates 21 models that participated in ISMIP6, projecting each of their contributions to global sea level rise.', echo=FALSE}
# Plot experiment output
ggplot() +
...
```{r fig.cap = 'Figure 4. The tlm.global experiment, plotted with 2000 climate input samples from both FaIR and Hector. The experiment simulates thermosteric sea level rise by using a two-layer model of the forcing-temperature coupling.', echo=FALSE}
# Plot experiment output
ggplot() +
    geom_ribbon(data = h_slr_stats,
                aes(x=year, ymin=min, ymax=max), color="#00BFC4", fill="#00BFC4",linetype=0,alpha=0.2) +
